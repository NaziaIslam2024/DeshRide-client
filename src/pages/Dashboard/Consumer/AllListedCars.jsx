...
const AllListedCars = () => {
  ...
  useEffect(() => {
    const fetchApprovedCars = async () => {
      try {
        ...
        if (filters.location)
          result = result.filter((car) => car.carLocation === filters.location);
        if (filters.minSeats)
          result = result.filter(
            (car) => car.seats >= Number(filters.minSeats)
          );
        ...
        const locations = [
          ...new Set(approvedCars.map((car) => car.carLocation)),
        ];
        const allFeatures = [
          ...new Set(approvedCars.flatMap((car) => car.features)),
        ];
  ...
  const applyFilters = () => {
    ...
    if (filters.location)
      result = result.filter((car) => car.carLocation === filters.location);
    if (filters.minSeats)
      result = result.filter((car) => car.seats >= Number(filters.minSeats));
    if (filters.features.length > 0) {
      result = result.filter((car) =>
        filters.features.every((feature) => car.features.includes(feature))
      );
