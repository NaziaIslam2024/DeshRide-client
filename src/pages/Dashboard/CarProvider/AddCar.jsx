...
const AddCar = () => {
  ...
  const onSubmit = async (data) => {
    try {
      ...
      // In your onSubmit function, add this to the FormData
  ...
  return (
    <div className="w-9/12 mx-auto p-5 text-center items-center bg-accent-light-100 rounded-lg mt-10">
      ...
      <form onSubmit={handleSubmit(onSubmit)} className="space-y-4">
        ...
        {/* // Add this to your form's grid section, just before the image upload */}
        <div className="form-control">
          <label className="label">
            <span className="label-text">Car Location</span>
          </label>
          <input
            type="text"
            {...register("carLocation", { required: "Location is required" })}
            placeholder="City or specific address"
            className="input input-bordered w-full"
          />
          {errors.carLocation && (
            <span className="text-red-500 text-sm">
              {errors.carLocation.message}
            </span>
          )}
        </div>
